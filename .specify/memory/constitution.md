...
6. **Core test-first, fetchers on fixtures** — MUST: deterministic logic is written test-first; fetchers get contract tests on recorded fixtures **that drive the real parse/extraction path — a loader mocked out in every test is untested, not tested**; no live network call in tests. The core is provable; the web is not.
...
- 1.2.0 (2026-07-08) — refined #6: fixtures MUST drive the real parse path, no fully-mocked loaders (from feature: m1-thicken-collection)
