...
- Q: Is a surviving role re-scored, and what governs model cost (FR-005/FR-019)? → A: Model features are cached so the formula recomputes with zero model calls on weight changes, and the model is re-called only when the role is new, changed, or stale (v1). A user-set per-role model-spend budget trading cost against quality is v2 (deferred — see FR-025). The eval guards cheap/weak models.
...
- **FR-025**: Model-supplied features MUST be cached so the fit formula recomputes with zero model calls when weights change, and the model is re-called only when a role is new, changed, or stale (v1 — the caching baseline). A user-set per-role model-spend budget that further governs re-scoring and model/provider escalation is **v2** (the budget knob, not the caching, is deferred).

### Key Entities *(include if feature involves data)*

- **Profile**: the user's CV (a single base CV in v1; a named variant library is v2), target lane, allowed location(s), compensation floor, output language, and one or more configured LLM providers (API key, subscription, or local); a per-role model-spend budget (cost/quality preference) is v2. Drives every scoring decision.
