"""LLM backend interface for the yoke pipeline.

The AI step is ONE narrow call: `complete(prompt, schema) -> dict`. Everything
around it (prepare.py, scoring.py, board.py) is deterministic and backend-agnostic.
...
class LLMBackend(ABC):
    ...
    def describe(self) -> str:
        """Human-readable backend id, e.g. 'claude-code:claude-haiku-4-5'."""
        return f"{self.name}:{getattr(self, 'model', '?')}"
